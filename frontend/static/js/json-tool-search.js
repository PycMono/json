...
function processJson() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('json-tool-search');
  clearErrorPanel();
  ...
  c.innerHTML = '<p class="jt-search-count">' + (i18n('json.search.found') || '找到') + ' ' + results.length + ' ' + (i18n('json.search.matches') || '处') + '</p>' +
    results.map(r => {
      return '<div class="jt-search-item">' +
        ...
        '<span class="jt-search-badge jt-search-badge--' + r.matchType + '">' + (r.matchType === 'key' ? (i18n('json.search.match_type.key') || '键') : (i18n('json.search.match_type.value') || '值')) + '</span>' +
