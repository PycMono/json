...
async function buyNumber() {
  ...
  try {
    ...
    if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('sms-order', 1, 0);
    if (typeof gaTrackPurchase === 'function') {
      var priceUSD = data.price ? parseFloat(data.price) / 7.0 : 0;
      gaTrackPurchase(
        (selectedService?.name || data.product || 'SMS'),
        priceUSD,
        String(data.id || currentOrderId)
      );
    }
    ...
    if (typeof gaTrackError === 'function') gaTrackError('sms-order', 'buy_failed', err.message);
    if (btn) { btn.disabled = false; btn.textContent = '🛒 获取号码'; }
  }
}
