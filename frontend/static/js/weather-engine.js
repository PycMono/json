...
async function initEngine() {
  ...
  document.getElementById('locateBtn')?.addEventListener('click', function() {
    locateUser();
    if (typeof gaTrackToolInteraction === 'function') gaTrackToolInteraction('weather', 'button_click');
  });
  document.getElementById('searchClearBtn')?.addEventListener('click', function() {
    clearSearch();
    if (typeof gaTrackToolInteraction === 'function') gaTrackToolInteraction('weather', 'button_click');
  });
  document.getElementById('shareSnapshotBtn')?.addEventListener('click', function() {
    saveSnapshot();
    if (typeof gaTrackToolInteraction === 'function') gaTrackToolInteraction('weather', 'button_click');
  });

  // Unit buttons
  document.querySelectorAll('[data-unit-temp]').forEach(b => b.addEventListener('click', () => {
    setUnit('temp', b.dataset.unitTemp);
    if (typeof gaTrackToolInteraction === 'function') gaTrackToolInteraction('weather', 'button_click');
  }));
  document.querySelectorAll('[data-unit-wind]').forEach(b => b.addEventListener('click', () => {
    setUnit('wind', b.dataset.unitWind);
    if (typeof gaTrackToolInteraction === 'function') gaTrackToolInteraction('weather', 'button_click');
  }));
  ...
  // Model compare
  document.getElementById('modelCompareToggle')?.addEventListener('change', e => {
    WeatherStore.modelCompareEnabled = e.target.checked;
    if (typeof gaTrackSettingChange === 'function') gaTrackSettingChange('weather', 'model_compare', e.target.checked);
  });
