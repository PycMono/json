...
;(function () {
  ...
  /**
   * 根据 resizeMode 绘制图片
   *   stretch — 强制拉伸到目标尺寸（默认）
   *   fit     — 按比例缩放，完整显示在目标框内，可能输出更小尺寸
   *   fill    — 按比例缩放填满目标框，按 cropFocus 裁剪多余部分
   */
  function drawWithMode(img, targetW, targetH, resizeMode, cropFocus) {
    ...
    if (mode === 'fill') {
      ...
      var dx = fillW - targetW;
      var dy = fillH - targetH;
  ...
  async function resizeFile(file, options) {
    ...
    if (typeof gaTrackToolUse === 'function') gaTrackToolUse('img-resize-engine');
    var id = uid();
    var loaded;
    try {
    ...
    var canvas;
    try {
      canvas = drawWithMode(img, dim.w, dim.h, options.resizeMode, options.cropFocus);
    } finally {
    ...
    if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('img-resize-engine', 1, Date.now() - _gaStart);
    return {
      id:       id,
      file:     file,
      blob:     blob,
  ...
  async function resizeBatch(files, options, onProgress) {
    ...
    if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('img-resize-engine', doneCount, Date.now() - _gaStart);
    return results;
  }
