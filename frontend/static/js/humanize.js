...
async function startHumanize() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('humanize');
...
function handleSSEData(eventType, data, outputEl) {
  if (eventType === 'message' || (!eventType && data.content)) {
    ...
    showOutputActions();
    if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('humanize', 1, Date.now() - _gaStart);
...
function copyOutput() {
  ...
  navigator.clipboard.writeText(text).then(() => {
    ...
    if (typeof gaTrackResultCopy === 'function') gaTrackResultCopy('humanize', 'text');
  }).catch(err => {
