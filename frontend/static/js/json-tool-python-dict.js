...
function initToolOptions() {
  ...
  el.innerHTML = `
    ...
    <div class="jt-options-row" style="gap:8px">
      <button class="jt-btn" id="btnToJson" onclick="setDirection('to-json')" style="background:#eef2ff">Python → JSON</button>
      <button class="jt-btn" id="btnToPy" onclick="setDirection('to-py')">JSON → Python</button>
    </div>
  `;
...
function processJson() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('json-tool-python-dict');
  clearErrorPanel();
  ...
  if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('json-tool-python-dict', 1, Date.now() - _gaStart);
    } else {
