...
async function startDetection() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('ailab-detector');
  ...
  try {
    ...
    // 3. 渲染结果
    renderResult(data);
      if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('ailab-detector', 1, Date.now() - _gaStart);              // A-04
...
function fetchURL() {
  ...
  .then(data => {
    ...
    if (data.success) {
      renderResult(data);
      if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('ailab-detector', 1, Date.now() - _gaStart);
