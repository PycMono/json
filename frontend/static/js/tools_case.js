  ...
  document.querySelectorAll('.case-copy-btn').forEach(btn => {
    btn.addEventListener('click', function () {
      ...
      navigator.clipboard.writeText(val).then(() => {
        ...
        if (typeof gaTrackResultCopy === 'function') gaTrackResultCopy('tools-case', 'text');
      });
    });
  });
}
...
document.getElementById('caseInput')?.addEventListener('focus', function () {
  if (typeof gaTrackToolInteraction === 'function') gaTrackToolInteraction('tools-case', 'input_focus');
});

document.getElementById('clearCaseBtn')?.addEventListener('click', function () {
  document.getElementById('caseInput').value = '';
  renderCards('');
  if (typeof gaTrackToolInteraction === 'function') gaTrackToolInteraction('tools-case', 'clear');
});
...
document.getElementById('copyAllCaseBtn')?.addEventListener('click', function () {
  ...
  navigator.clipboard.writeText(JSON.stringify(result, null, 2)).then(() => {
    ...
    if (typeof gaTrackResultCopy === 'function') gaTrackResultCopy('tools-case', 'json');
  });
});
