...
function processJson() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('json-tool-from-toml');
  clearErrorPanel();
  const input = getInput();
  if (!input.trim()) {
    showToast(i18n('json.common.error.empty') || '请先输入 TOML 内容', 'error');
    return;
  }
  ...
  if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('json-tool-from-toml', 1, Date.now() - _gaStart);
