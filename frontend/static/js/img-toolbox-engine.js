...
const ITB_MAX_SIZE_MB   = 100;  // 纯前端 Canvas 处理，100MB 以内图片均可处理
...
function itbAddFiles(fileList) {
  ...
  for (const file of files) {
    ...
    if (file.size > ITB_MAX_SIZE_MB * 1024 * 1024) {
      ...
      itbToast(`${file.name} (${fileSizeMB}MB): ` + itbI18n('error.common.file_too_large') + ` (max ${ITB_MAX_SIZE_MB}MB)`, 'error');
      continue;
    }
...
async function itbStartProcess() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('img-toolbox-engine');
  ...
  if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('img-toolbox-engine', done.length, Date.now() - _gaStart);
}
...
async function itbProcessOne(entry, opts) {
  ...
  try {
    ...
    switch (ITB_TOOL) {
      ...
      case 'remove-bg':
        // remove-bg AI/CDN logic lives in remove-bg-engine.js (loaded only on that page)
        if (typeof window._itbProcessRemoveBg !== 'function') {
          throw new Error(itbI18n('error.remove_bg.model_load_failed') || 'remove-bg engine not loaded');
        }
        outputBlob = await window._itbProcessRemoveBg(entry, opts);
        break;
...
/* ───────────────────── REMOVE BG ────────────────────────── */
/* AI background removal logic moved to remove-bg-engine.js (loaded only on
   /img/remove-bg). The shared engine dispatches to window._itbProcessRemoveBg. */
...
async function itbProcessWatermark(entry, opts) {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (opts.wmType === 'none') {
        _itbWmExport(canvas, entry, opts, resolve, reject);
        return;
      }
      ...
      if (opts.wmType === 'tile') {
        itbDrawTileWatermark(ctx, canvas.width, canvas.height, opts);
        _itbWmExport(canvas, entry, opts, resolve, reject);
      } else if (opts.wmType === 'image' && opts.wmImgURL) {
        const wi = new Image();
        wi.onload = () => {
          const baseW = Math.min(canvas.width * 0.3, wi.naturalWidth);
          const ww = baseW * (opts.wmScale || 100) / 100;
          const wh = ww / wi.naturalWidth * wi.naturalHeight;
          const [px, py] = itbWmPos(canvas.width, canvas.height, opts.wmPosition || 'br', ww, wh, opts.wmPadding || 20);
          ctx.save();
          ctx.translate(px + ww/2, py - wh/2);
          ctx.rotate((opts.wmRotation || 0) * Math.PI / 180);
        ...
        const fs = (opts.wmFontSize || 32) * (opts.wmScale || 100) / 100;
        ctx.font = `bold ${fs}px Arial, sans-serif`;
        ctx.fillStyle = opts.wmColor || '#ffffff';
        const text = opts.wmText || 'Watermark';
        const tw = ctx.measureText(text).width;
        const th = fs;
        const [px, py] = itbWmPos(canvas.width, canvas.height, opts.wmPosition || 'br', tw, th, opts.wmPadding || 20);
        ctx.save();
...
function _itbWmExport(canvas, entry, opts, resolve, reject) {
  let mime = entry.file.type || 'image/jpeg';
  const fmt = opts.outputFormat || 'original';
  ...
  const quality = (mime === 'image/jpeg' || mime === 'image/webp')
    ? ((opts.outputQuality || 85) / 100)
    : undefined;
  canvas.toBlob(b => b ? resolve(b) : reject(new Error('toBlob failed')), mime, quality);
}

function itbDrawTileWatermark(ctx, cw, ch, opts) {
  ...
  const tw = ctx.measureText(text).width;
  const th = fs;
  const spacing = opts.wmSpacing || 150;
  const angle = (opts.wmTileAngle || -30) * Math.PI / 180;
...
function itbWmPos(cw, ch, pos, ww, wh, pad) {
...
async function itbProcessRotate(entry, opts) {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      let bgColor = opts.bgColor || '#FFFFFF';
      if (fmt === 'png' || fmt === 'webp' || (fmt === 'original' && (mime === 'image/png' || mime === 'image/webp'))) {
        // For transparent-capable formats, if bgColor is explicitly transparent or not set, keep transparent
...
function itbDownloadOne(id) {
  ...
  a.click();
  if (typeof gaTrackDownload === 'function') gaTrackDownload('img-toolbox-engine', entry.file.type || 'image/jpeg');
}
...
async function itbDownloadAll() {
  ...
  if (typeof gaTrackDownloadAll === 'function') gaTrackDownloadAll('img-toolbox-engine', done.length);
}
...
function itbBuildOutputName(entry, opts, outMime) {
  ...
  const naming = opts.naming || 'same';
  let suffix = '';
  if (naming === 'suffix-rotated') suffix = '-rotated';
  else if (naming === 'suffix-fixed') suffix = '-fixed';
  else if (naming === 'suffix-watermarked') suffix = '-watermarked';
