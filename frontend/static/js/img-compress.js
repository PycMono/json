...
function showToast(msg, type) {
  ...
  setTimeout(function(){ t.style.opacity='0'; setTimeout(function(){ t.remove(); },250); }, duration);
}

/* ── 上传区持久错误提示（文件被拒时显示，直到下次成功上传才消失）── */
function showUploadZoneError(msg) {
  var zone = document.getElementById('dropZone');
  if (!zone) return;
  var old = zone.querySelector('.ic-drop-error');
  if (old) old.remove();
  var el = document.createElement('div');
  el.className = 'ic-drop-error';
  el.style.cssText = 'margin-top:8px;padding:6px 12px;background:#fef2f2;border:1px solid #fca5a5;border-radius:6px;color:#dc2626;font-size:13px;font-weight:500;text-align:center;';
  el.textContent = '⚠ ' + msg;
  zone.appendChild(el);
  // 10 秒后自动消失
...
function onFilesAdded() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('img-compress');
  showResultsSection();
  startCompress().then(function() {
    if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('img-compress', 1, Date.now() - _gaStart);
  });
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function _applyFmt(recompress) {
    ...
    var v = fmt.value;
    ICState.outputFormat = v === 'auto' ? 'original' : v === 'jpg' ? 'image/jpeg' : v === 'png' ? 'image/png' : v === 'webp' ? 'image/webp' : v === 'gif' ? 'image/gif' : v === 'bmp' ? 'image/bmp' : v === 'tiff' ? 'image/tiff' : 'original';
