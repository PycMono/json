...
function copyEditorContent() {
    ...
    navigator.clipboard.writeText(text).then(() => {
        if (typeof gaTrackResultCopy === 'function') gaTrackResultCopy('json-editor', 'text');
...
function downloadJSON() {
    ...
    if (typeof gaTrackDownload === 'function') gaTrackDownload('json-editor', 'json');
}
...
async function fetchJSONFromURL() {
    ...
    if (typeof gaTrackToolUse === 'function') gaTrackToolUse('json-editor-fetch-url');
    ...
    try {
        ...
        if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('json-editor-format', 1, Date.now() - _gaStart);
                showValidStatus(result.parsed, formatted);
        if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('json-editor-validate', 1, Date.now() - _gaStart);
            } else {
        ...
        if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('json-editor-fetch-url', 1, Date.now() - _gaStart);
        }
    } catch (err) {
...
function validateJSON() {
    var _gaStart = Date.now();
    if (typeof gaTrackToolUse === 'function') gaTrackToolUse('json-editor-validate');
    if (!editor) return;
...
function formatJSON() {
    var _gaStart = Date.now();
    if (typeof gaTrackToolUse === 'function') gaTrackToolUse('json-editor-format');
    if (!editor) return;
    const text = editor.getValue().trim();
    if (!text) return;
...
function minifyJSON() {
    var _gaStart = Date.now();
    if (typeof gaTrackToolUse === 'function') gaTrackToolUse('json-editor-minify');
    if (!editor) return;
    const text = editor.getValue().trim();
    if (!text) return;
    ...
    editor.setValue(minified);
        if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('json-editor-minify', 1, Date.now() - _gaStart);
