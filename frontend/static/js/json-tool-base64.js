...
function initToolOptions() {
  const target = document.getElementById('inputOptions') || document.getElementById('inputOptions');
  if (!target) return;
  target.innerHTML =
...
function processJson() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('json-tool-base64'); encodeJson(); }
function encodeJson() {
  clearErrorPanel();
...
function decodeJson() {
  ...
  if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('json-tool-base64', 1, Date.now() - _gaStart);
    }
    const el = document.getElementById('outputStats');
