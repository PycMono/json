...
async function compressSingle(id) {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('tools_image_compress');
  ...
  try {
    ...
    img.status = 'done';
    if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('tools_image_compress', 1, Date.now() - _gaStart);
  } catch (error) {
...
function downloadSingle(id) {
  ...
  if (typeof gaTrackDownload === 'function') gaTrackDownload('tools_image_compress', img.compressedBlob.type || 'image/jpeg');
...
document.getElementById('downloadAllBtn')?.addEventListener('click', async () => {
  ...
  if (typeof gaTrackDownload === 'function') gaTrackDownload('tools_image_compress', 'application/zip');
  a.click();
  URL.revokeObjectURL(url);
});
