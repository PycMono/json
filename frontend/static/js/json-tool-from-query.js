...
function processJson() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('json-tool-from-query');
  clearErrorPanel();
  const input = getInput();
  if (!input.trim()) {
    showToast(i18n('json.common.error.empty') || '请先输入内容', 'error');
    return;
  }
  ...
  if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('json-tool-from-query', 1, Date.now() - _gaStart);
    const statsEl = document.getElementById('outputStats');
    if (statsEl) {
      const keyCount = Object.keys(result).length;
      statsEl.innerHTML = '<span class="jt-success-badge">✅ ' + keyCount + ' fields parsed</span>';
    }
  } catch (err) {
    showErrorPanel(err, input);
  }
}
