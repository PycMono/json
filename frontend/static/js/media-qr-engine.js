...
function generateQR() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('qr-generator');
  ...
  if (txt) txt.textContent = (window.QR_I18N && window.QR_I18N.generating) || '生成中...';
  ...
  try {
    ...
    if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('qr-generator', 1, Date.now() - _gaStart);

    if (typeof renderScanTips === 'function') renderScanTips(data);

    if (QRState.design.frame) {
      setTimeout(renderFrameComposite, 300);
    }
  } catch (err) {
    showToast(((window.QR_I18N && window.QR_I18N.generateFailed) || '生成失败：') + err.message, 'error');
    if (typeof gaTrackError === 'function') gaTrackError('media-qr', 'generate_failed', err.message);
  } finally {
    setTimeout(() => {
      if (btn) btn.disabled = false;
      if (txt) txt.textContent = (window.QR_I18N && window.QR_I18N.generateBtn) || '生成二维码';
    }, 400);
  }
}
...
function updatePreview() {
  ...
  updatePreviewTimer = setTimeout(() => {
    ...
    if (typeof gaTrackSettingChange === 'function') gaTrackSettingChange('media-qr', 'design', 'update');
  }, 100);
}
...
async function downloadQR(format) {
  if (!QRState.hasGenerated) {
    showToast((window.QR_I18N && window.QR_I18N.empty) || '请先生成二维码', 'error'); return;
  }
  ...
  switch (format) {
    ...
    default: showToast((window.QR_I18N && window.QR_I18N.formatUnsupported) || '不支持的格式', 'error');
  }
}
...
async function downloadPNG(filename) {
  ...
  try {
    ...
    if (!canvas) { showToast((window.QR_I18N && window.QR_I18N.pngFailed) || 'PNG 生成失败', 'error'); return; }
    canvas.toBlob(blob => {
      saveAs(blob, `${filename}.png`);
      showToast((window.QR_I18N && window.QR_I18N.pngSuccess) || 'PNG 下载成功！', 'success');
      if (typeof gaTrackDownload === 'function') gaTrackDownload('media-qr', 'image/png');
    }, 'image/png');
  } finally {
...
async function downloadJPG(filename) {
  ...
  try {
    ...
    if (!srcCanvas) { showToast((window.QR_I18N && window.QR_I18N.jpgFailed) || 'JPG 生成失败', 'error'); return; }
    ...
    canvas.toBlob(blob => {
      ...
      showToast((window.QR_I18N && window.QR_I18N.jpgSuccess) || 'JPG 下载成功！', 'success');
      if (typeof gaTrackDownload === 'function') gaTrackDownload('media-qr', 'image/jpeg');
    }, 'image/jpeg', 0.92);
  } finally {
...
async function downloadSVG(filename) {
  ...
  try {
    ...
    if (!svgStr) { showToast((window.QR_I18N && window.QR_I18N.svgFailed) || 'SVG 生成失败', 'error'); return; }
    const blob = new Blob([svgStr], { type: 'image/svg+xml;charset=utf-8' });
    saveAs(blob, `${filename}.svg`);
    showToast((window.QR_I18N && window.QR_I18N.svgSuccess) || 'SVG 下载成功！', 'success');
    if (typeof gaTrackDownload === 'function') gaTrackDownload('media-qr', 'image/svg+xml');
  } finally {
...
async function downloadEPS(filename) {
  ...
  try {
    const svgStr = await getQRSVGString(500);
    if (!svgStr) { showToast((window.QR_I18N && window.QR_I18N.epsFailed) || 'EPS 生成失败', 'error'); return; }
    ...
    if (!resp.ok) { showToast((window.QR_I18N && window.QR_I18N.epsRetry) || 'EPS 生成失败，请稍后重试', 'error'); return; }

    const blob = await resp.blob();
    saveAs(blob, `${filename}.eps`);
    showToast((window.QR_I18N && window.QR_I18N.epsSuccess) || 'EPS 下载成功！', 'success');
    if (typeof gaTrackDownload === 'function') gaTrackDownload('media-qr', 'application/postscript');
  } catch(e) {
    showToast(((window.QR_I18N && window.QR_I18N.epsError) || 'EPS 下载失败：') + e.message, 'error');
    if (typeof gaTrackError === 'function') gaTrackError('media-qr', 'eps_download_failed', e.message);
  } finally {
...
async function copyQRImage() {
  if (!QRState.hasGenerated) { showToast((window.QR_I18N && window.QR_I18N.empty) || '请先生成二维码', 'error'); return; }
  ...
  try {
    ...
    canvas.toBlob(async blob => {
      try {
        ...
        showToast((window.QR_I18N && window.QR_I18N.copySuccess) || '已复制到剪贴板！', 'success');
        if (typeof gaTrackResultCopy === 'function') gaTrackResultCopy('media-qr', 'image');
        const btn = document.getElementById('copyBtn');
        if (btn) {
          const orig = btn.textContent;
          btn.textContent = (window.QR_I18N && window.QR_I18N.copyDone) || '✅ 已复制';
          setTimeout(() => { btn.textContent = orig; }, 2000);
        }
      } catch(e) {
        saveAs(blob, 'qrcode.png');
        showToast((window.QR_I18N && window.QR_I18N.copyFallback) || '已为您下载（浏览器不支持复制）', 'info');
      }
    }, 'image/png');
  } catch(e) {
    showToast((window.QR_I18N && window.QR_I18N.copyFailed) || '复制失败', 'error');
  }
}
