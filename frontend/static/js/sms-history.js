...
function filterByStatus(status, btn) {
  ...
  if (typeof gaTrackTabSwitch === 'function') gaTrackTabSwitch(status, 'history_status', 'sms-history');
...
function clearHistory() {
  ...
  if (typeof gaTrackFormSubmit === 'function') gaTrackFormSubmit('clear_history', 'sms-history');
...
/* ── 导出 CSV ───────────────────────────────── */
function exportCSV() {
  if (typeof gaTrackExport === 'function') gaTrackExport('sms-history', 'csv');
  const list = getHistory();
  if (list.length === 0) {
