...
function loadExcelSheet(index) {
  ...
  setOutput(output);
  if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('json-tool-from-excel', 1, Date.now() - _gaStart);
...
function processJson() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('json-tool-from-excel'); showToast(i18n('json.convert.upload_excel') || '请上传 Excel 文件', 'info'); }
