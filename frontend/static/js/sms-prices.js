...
async function loadPricePage(page) {
  ...
  try {
    ...
    updatePriceMeta();
    if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('sms-prices', data.prices?.length || 0, performance.now() - t0);
  } catch (err) {
    console.error('加载价格失败:', err);
    showPriceError(err.message);
    if (typeof gaTrackError === 'function') gaTrackError('sms-prices', 'load_fail', err.message);
  } finally {
...
function showPriceHistory(priceKey) {
  if (typeof gaTrackModalOpen === 'function') gaTrackModalOpen('price_history', 'sms-prices');
  const hist = priceHistory[priceKey];
  ...
  modal.onclick = e => { if(e.target===modal) { modal.remove(); if (typeof gaTrackModalClose === 'function') gaTrackModalClose('price_history', 'sms-prices'); } };
  modal.innerHTML = `
    <div class="price-history-content">
      <div class="price-history-header">
        <h3>${platform} · ${country}</h3>
        <button onclick="document.getElementById('price-history-modal').remove(); if (typeof gaTrackModalClose === 'function') gaTrackModalClose('price_history', 'sms-prices');" class="close-btn">✕</button>
      </div>
