...
function processJson() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('json-tool-schema-validate');
  clearErrorPanel();
  ...
  if (!schemaRaw) { showToast(i18n('json.schema.validate.error.empty_schema') || '请先在左侧填写 JSON Schema', 'error'); return; }
  if (!dataRaw)   { showToast(i18n('json.schema.validate.error.empty_data') || '请先在右侧填写要验证的 JSON 数据', 'error'); return; }

  let schema, data;
  try { schema = JSON.parse(schemaRaw); }
  catch(e) {
    showToast(i18n('json.schema.validate.error.invalid_schema') || 'Schema JSON 格式错误', 'error');
    // Highlight the schema editor
    if (window._schemaEditor) {
      const msg = e.message || String(e);
      const el = document.getElementById('schemaResultPanel');
      const body = document.getElementById('schemaResultBody');
      const parseFailedKey = 'json.schema.validate.error.parse_failed';
      const parseFailedText = i18n(parseFailedKey) || 'Schema JSON 解析失败';
      if (el) el.style.display = '';
      if (body) body.innerHTML =
        '<div class="jt-error-location" style="padding:12px 16px">❌ ' + parseFailedText + '：<strong>' +
        escapeHtml(msg) + '</strong></div>';
      window._schemaResultText = parseFailedText + '：' + msg;
    }
    return;
  }
  ...
  if (errors.length === 0) {
    const successTitle = i18n('json.schema.validate.success.title') || '验证通过';
    const successMsg = i18n('json.schema.validate.success.message') || 'JSON 数据完全符合 Schema 规则，无任何错误。';
    const html = '<div style="padding:16px 20px;display:flex;align-items:center;gap:10px">' +
      '<span style="font-size:2rem">✅</span>' +
      '<div><div style="font-weight:700;font-size:1rem;color:#065f46">' + successTitle + '</div>' +
      '<div style="color:#6b7280;font-size:0.875rem;margin-top:2px">' + successMsg + '</div></div>' +
      '</div>';
    _showResult(html, '✅ ' + successTitle + '\n\n' + successMsg);
    if (statsEl) statsEl.innerHTML = '<span class="jt-success-badge">✅ ' + successTitle + '</span>';
  } else {
    const errorTitle = i18n('json.schema.validate.error.title') || '验证失败';
    const foundErrorsKey = 'json.schema.validate.error.found_errors';
    const foundErrorsText = (i18n(foundErrorsKey) || '共发现 {n} 个错误').replace('{n}', errors.length);
    const pathLabel = i18n('json.schema.validate.error.path_label') || '路径';
    const xErrorsKey = 'json.schema.validate.error.x_errors';
    const xErrorsText = (i18n(xErrorsKey) || '{n} 个错误').replace('{n}', errors.length);

    const lines = ['❌ ' + errorTitle + '，' + foundErrorsText + '：', ''];
    const rows = errors.map(function(e, i) {
      lines.push((i + 1) + '. ' + pathLabel + ': ' + e.path);
      lines.push('   ' + e.msg);
      lines.push('');
    ...
    const html =
      ...
      '❌ ' + errorTitle + '，' + foundErrorsText + '</div>' +
      '<div style="padding:12px">' + rows + '</div>';
    _showResult(html, lines.join('\n'));
    if (statsEl) statsEl.innerHTML = '<span style="color:#dc2626;font-size:0.8rem;font-weight:700">❌ ' + xErrorsText + '</span>';
  }
}
