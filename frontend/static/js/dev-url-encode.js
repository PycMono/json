...
const DevURLEncode = (() => {
  ...
  function compute() {
    ...
    if (STATE.batchMode) {
      ...
      if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('dev-url-encode', results.length, durationMs);
    } else {
      try {
        setOutput(fn(input), input);
        const durationMs = Math.round(performance.now() - startMs);
        if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('dev-url-encode', 1, durationMs);
      } catch (e) {
        if (typeof gaTrackError === 'function') gaTrackError('dev-url-encode', 'decode_error', e.message);
  ...
  async function copyOutput() {
    ...
    if (typeof gaTrackResultCopy === 'function') gaTrackResultCopy('dev-url-encode', 'text');
  ...
  function setMode(m) {
    ...
    if (typeof gaTrackSettingChange === 'function') gaTrackSettingChange('dev-url-encode', 'mode', m);
    compute();
  }

  function setMethod(m) {
    STATE.method = m;
    if (typeof gaTrackSettingChange === 'function') gaTrackSettingChange('dev-url-encode', 'method', m);
    compute();
  }
