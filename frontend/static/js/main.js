...
function switchLang(lang) {
  if (typeof gaTrackLanguageChange === 'function') {
    gaTrackLanguageChange(document.documentElement.lang || 'en', lang);
  }
  ...
  fetch(`/api/search?q=${encodeURIComponent(q)}`)
    ...
    .then(data => {
      if (!data.results || data.results.length === 0) {
        navSearchResults.classList.remove('show');
        if (typeof gaTrackSiteSearch === 'function') gaTrackSiteSearch('nav', q, 0);
        return;
      }
      if (typeof gaTrackSiteSearch === 'function') gaTrackSiteSearch('nav', q, data.results.length);
