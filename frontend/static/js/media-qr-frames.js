  ...
  body.innerHTML = `
    <div class="qr-frame-grid" id="frameGrid">
      ...
      <label class="qr-frame-item qr-frame-item--active">
        ...
        <span class="qr-frame-label">${_qf('frameLabelNone','无边框')}</span>
      </label>
      ...
      <label class="qr-frame-item">
        ...
        <span class="qr-frame-label">${_qf('frameLabelRectBottom','底部文字')}</span>
      </label>
      ...
      <label class="qr-frame-item">
        ...
        <span class="qr-frame-label">${_qf('frameLabelRectTop','顶部文字')}</span>
      </label>
      ...
      <label class="qr-frame-item">
        ...
        <span class="qr-frame-label">${_qf('frameLabelRoundedBottom','圆角底字')}</span>
      </label>
      ...
      <label class="qr-frame-item">
        ...
        <span class="qr-frame-label">${_qf('frameLabelBubble','气泡框')}</span>
      </label>
      ...
      <label class="qr-frame-item">
        ...
        <span class="qr-frame-label">${_qf('frameLabelBannerTop','彩色顶条')}</span>
      </label>
      ...
      <label class="qr-frame-item">
        ...
        <span class="qr-frame-label">${_qf('frameLabelBannerBottom','彩色底条')}</span>
      </label>
      ...
      <label class="qr-frame-item">
        ...
        <span class="qr-frame-label">${_qf('frameLabelDoubleBorder','双线边框')}</span>
      </label>
    ...
    <div class="qr-frame-opts" id="frameOpts" style="display:none">
      <div class="qr-option-row">
        <label class="qr-option-label">${_qf('frameOptColor','边框颜色')}</label>
      ...
      <div class="qr-option-row" id="frameLabelRow">
        <label class="qr-option-label">${_qf('frameOptText','标签文字')}</label>
      ...
      <div class="qr-option-row" id="frameFontRow">
        <label class="qr-option-label">${_qf('frameOptFontSize','字号')}</label>
...
async function downloadQR(format) {
  if (!QRState.hasGenerated) {
    showToast(_qf('frameEmpty','请先生成二维码'), 'error');
    return;
  }
  ...
  try {
    switch (format) {
      ...
      case 'png': {
        ...
        if (!c) { showToast(_qf('pngFailed','PNG 生成失败'), 'error'); break; }
        c.toBlob(b => {
          saveAs(b, `${fn}.png`);
          showToast(_qf('pngSuccess','PNG 下载成功！'), 'success');
          if (typeof gaTrackDownload === 'function') gaTrackDownload('qr-generator', 'png');
        }, 'image/png');
        break;
      }
      ...
      case 'jpg': {
        ...
        if (!src) { showToast(_qf('jpgFailed','JPG 生成失败'), 'error'); break; }
        ...
        out.toBlob(b => {
          saveAs(b, `${fn}.jpg`);
          showToast(_qf('jpgSuccess','JPG 下载成功！'), 'success');
          if (typeof gaTrackDownload === 'function') gaTrackDownload('qr-generator', 'jpg');
        }, 'image/jpeg', 0.92);
        break;
      }
      ...
      case 'svg': {
        ...
        if (!svg) { showToast(_qf('svgFailed','SVG 生成失败'), 'error'); break; }
        saveAs(new Blob([svg], { type: 'image/svg+xml;charset=utf-8' }), `${fn}.svg`);
        showToast(_qf('svgSuccess','SVG 下载成功！'), 'success');
        if (typeof gaTrackDownload === 'function') gaTrackDownload('qr-generator', 'svg');
        break;
      }

      case 'eps': {
        const svg = await _getSVGWithFrame(500);
        if (!svg) { showToast(_qf('epsFailed','EPS 生成失败'), 'error'); break; }
        const resp = await fetch('/media/qr/api/eps', {
          method:  'POST',
          headers: { 'Content-Type': 'application/json' },
          body:    JSON.stringify({ svg }),
        });
        if (!resp.ok) { showToast(_qf('epsRetry','EPS 生成失败，请稍后重试'), 'error'); break; }
        saveAs(await resp.blob(), `${fn}.eps`);
        showToast(_qf('epsSuccess','EPS 下载成功！'), 'success');
        if (typeof gaTrackDownload === 'function') gaTrackDownload('qr-generator', 'eps');
        break;
      }

      default:
        showToast(_qf('frameDlUnsupported','不支持的格式'), 'error');
    }
  } catch (e) {
    showToast(`${_qf('frameDlFailPrefix','下载失败：')}${e.message}`, 'error');
  } finally {
