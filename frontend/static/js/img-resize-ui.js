...
;(function () {
  ...
  var IR_MAX_SIZE_MB = 100; // 纯前端 Canvas，100MB 以内均可处理
  ...
  function handleFiles(files) {
    ...
    // 大小检查（之前缺失，导致大文件 Canvas 静默失败）
    var oversized = valid.filter(function(f) { return f.size > IR_MAX_SIZE_MB * 1024 * 1024; });
    if (oversized.length) {
      oversized.forEach(function(f) {
        var mb = (f.size / (1024 * 1024)).toFixed(1);
        showToast(f.name + ' (' + mb + 'MB) 超过 ' + IR_MAX_SIZE_MB + 'MB 限制', 'error');
      });
      valid = valid.filter(function(f) { return f.size <= IR_MAX_SIZE_MB * 1024 * 1024; });
  ...
  /* ═══════════ 调整模式切换（Fit / Fill / Stretch） ═══════════ */
  function initResizeMode() {
    var tabs = document.querySelectorAll('.ir-resize-mode-tab');
  ...
  window.switchResizeMode = function (mode) {
    state.resizeMode = mode;
    document.querySelectorAll('.ir-resize-mode-tab').forEach(function (b) {
      b.classList.toggle('ir-resize-mode-tab--active', b.dataset.mode === mode);
    });
    var hint = document.getElementById('resizeModeHint');
    if (hint) {
      hint.textContent = hint.getAttribute('data-' + mode) || '';
    }
    var cropPanel = document.getElementById('cropFocusPanel');
    if (cropPanel) {
      cropPanel.style.display = mode === 'fill' ? '' : 'none';
    }
  };

  window.setCropFocus = function (focus) {
    state.cropFocus = focus;
    document.querySelectorAll('.ir-crop-focus__cell').forEach(function (b) {
      b.classList.toggle('ir-crop-focus__cell--active', b.dataset.focus === focus);
    });
  };
  ...
  function collectOptions() {
    ...
    var opts = { mode: state.mode, format: fe ? fe.value : '', quality: qs ? parseInt(qs.value)/100 : 0.9, lockRatio: state.lockRatio, resizeMode: state.resizeMode, cropFocus: state.cropFocus };
  ...
  window.startResize = async function () {
    ...
    if (typeof gaTrackToolUse === 'function') gaTrackToolUse('img-resize-ui');
    ...
    if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('img-resize-ui', Object.keys(state.cardMeta).length, Date.now() - _gaStart);
  ...
  window.downloadOne = function (id) {
    ...
    if (typeof gaTrackDownload === 'function') gaTrackDownload('img-resize-ui', 'image/jpeg');
  ...
  window.downloadAll = async function () {
    ...
    if (typeof gaTrackDownload === 'function') gaTrackDownload('img-resize-ui', 'application/zip');
    if (metas.length === 1) {
