...
function initToolOptions() {
  ...
  el.innerHTML = `
    ...
    <div class="jt-options-row" style="gap:8px">
      <button class="jt-btn jt-btn--primary" id="btnHtml" onclick="setExportFormat('html')">HTML</button>
      <button class="jt-btn" id="btnRtf" onclick="setExportFormat('rtf')">RTF</button>
    </div>
...
function processJson() {
  ...
  if (typeof gaTrackToolUse === 'function') gaTrackToolUse('json-tool-highlight-export');
  clearErrorPanel();
...
${highlighted}
    ...
    setOutput(html, 'html');
  if (typeof gaTrackProcessDone === 'function') gaTrackProcessDone('json-tool-highlight-export', 1, Date.now() - _gaStart);
  } else {
    // RTF format
