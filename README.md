...
## SMS 本地库初始化（feature_v41）

SMS 读接口已改为优先读取 MySQL 本地表（`sms_country`/`sms_product`/`sms_price`）。

### 1) 生成全量 SQL（从 5SIM 拉取）
...
- `scripts/sql/country.sql`
- `scripts/sql/product.sql`
- `scripts/sql/price.sql`

### 2) 导入顺序

```bash
mysql -u root -p toolboxnova < scripts/sql/schema.sql
mysql -u root -p toolboxnova < scripts/sql/country.sql
mysql -u root -p toolboxnova < scripts/sql/product.sql
mysql -u root -p toolboxnova < scripts/sql/price.sql
```

### 3) 冷启动行为

- 启动时会执行 `AutoMigrate`。
- 启动时会执行一次 5SIM 与本地库数量对账（country/product/price）。
- 若对账不一致则执行 upsert 同步；若对账失败则回退执行一次 `SyncAll`。
