...
func PrivacyCheckPage(c *gin.Context) {
	...
	// Build FAQ from i18n via centralized package
	...
	data := render.BaseData(c, gin.H{
		...
		"SEOArticle":       template.HTML(t("pc.seo.article")),
		"FAQs":             faqs,
		"TurnstileSiteKey": "",
		"JSONLD": template.JS(`{
  ...
  "aggregateRating": {
	...
	render.Render(c, "privacy/check.html", data)
}
...
func PrivacyCheckEmail(c *gin.Context) {
	...
	case 401:
		// No API key configured — treat as safe (graceful degradation)
		c.JSON(200, gin.H{
			"fallback": false,
			"breaches": []interface{}{},
			"safe":     true,
	...
	default:
		// 403, 5xx — all fallback gracefully
