...
// GeminiImpl Google Gemini REST API 实现
...
func NewGeminiImpl(cfg PlatformConf) *GeminiImpl {
	if cfg.BaseURL == "" {
...
func (p *GeminiImpl) GetProviderName() string { return "gemini" }
func (p *GeminiImpl) IsAvailable() bool       { return p.cfg.APIKey != "" }
...
func (p *GeminiImpl) Chat(ctx context.Context, req ChatRequest) (*ChatResponse, error) {
	start := time.Now()
...
func (p *GeminiImpl) ChatStream(ctx context.Context, req ChatRequest) (<-chan StreamChunk, error) {
	gemReq := p.buildRequest(req)
	body, _ := json.Marshal(gemReq)
...
func (p *GeminiImpl) buildRequest(req ChatRequest) geminiRequest {
...
// GenerateImage is not supported by Gemini - returns error
func (p *GeminiImpl) GenerateImage(ctx context.Context, req ImageRequest) (*ImageResponse, error) {
