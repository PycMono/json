...
func setupLocalesB(t *testing.T) string {
	...
	structure := map[string]map[string]string{
		"en": {
			"common.json":     `{"site.title": "json", "button.submit": "Submit"}`,
...
func newManager(t *testing.T, dir string, extraCfg ...func(*i18n2.Config)) *i18n2.Manager {
	t.Helper()
	cfg := i18n2.Config{
		LocalesDir:  dir,
		DefaultLang: "en",
...
func TestManager_Fallback_Chain(t *testing.T) {
	dir := setupLocales(t)
	m := newManager(t, dir, func(c *i18n2.Config) {
...
func TestManager_Reload(t *testing.T) {
	...
	tr := m.Translator("en")
	if got := tr.T("site.title"); got != "json" {
...
func TestDetector_PrefixMatch(t *testing.T) {
	...
	d := i18n2.NewDetector([]string{"en", "zh", "pt-PT"}, "en")
...
func newDetector(langs []string, def string) *i18n2.Detector {
	return i18n2.NewDetector(langs, def)
}
...
func TestIntegration_MiddlewarePattern(t *testing.T) {
	dir := setupLocales(t)
	m := newManager(t, dir)
	detector := i18n2.NewDetector(m.SupportedLangs(), "en")
...
func TestIntegration_RealI18nFiles(t *testing.T) {
	...
	cases := []struct{ lang, key, want string }{
		{"en", "home.title", "json - Free Online Tools for Developers & Privacy"},
...
func TestWatcher_HotReload(t *testing.T) {
	...
	if got := m.Translator("en").T("site.title"); got != "json" {
...
func BenchmarkTranslator_T(b *testing.B) {
	...
	m, _ := i18n2.NewManager(i18n2.Config{LocalesDir: dir, DefaultLang: "en"})
	tr := m.Translator("zh")
