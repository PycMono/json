...
// JSONToolFAQs returns FAQs for JSON tools by language.
func JSONToolFAQs(lang string) []FAQ {
	if lang == "zh" {
		return []FAQ{
			{Q: "什么是 JSON？", A: "JSON（JavaScript Object Notation）是一种轻量级的数据交换格式，易于人类阅读和编写，也易于机器解析和生成。它基于 JavaScript 的一个子集，但独立于编程语言，被广泛用于 Web API 和配置文件。"},
			{Q: "如何校验 JSON 是否有效？", A: "将 JSON 文本粘贴到上方编辑器，然后点击「校验 JSON」按钮。工具会立即检测语法错误，并精确指出错误所在的行和列号，附带友好的错误说明。"},
			{Q: "JSON 格式化有什么用？", A: "格式化（美化）JSON 会添加适当的缩进和换行，使嵌套结构更易于阅读和调试。点击「格式化」按钮即可一键美化，支持 2 空格、4 空格或 Tab 缩进。"},
			{Q: "JSON 压缩（Minify）是什么？", A: "JSON 压缩会移除所有多余的空格和换行符，将 JSON 压缩为最小体积的单行文本。这在生产环境中可以减少网络传输体积，加快 API 响应速度。"},
			{Q: "JSON 中常见的语法错误有哪些？", A: "最常见的错误包括：键名未用双引号包裹、末尾多余的逗号、使用单引号替代双引号、数组或对象括号不匹配、数字前有前导零等。本工具会精确指出每种错误的位置和原因。"},
			{Q: "数据会上传到服务器吗？", A: "不会。所有 JSON 的校验、格式化和压缩操作均在您的浏览器本地完成，数据不会发送到任何服务器，完全保护您的数据隐私。"},
			{Q: "如何从 URL 导入 JSON？", A: "切换到「抓取 URL」标签，输入包含 JSON 的网址，点击「抓取」按钮。工具会通过服务器代理获取内容并填入编辑器，随后自动格式化展示。"},
		}
	}
	return []FAQ{
		{Q: "What is JSON?", A: "JSON (JavaScript Object Notation) is a lightweight, text-based data interchange format that is easy to read and write for humans and easy to parse for machines. Though derived from JavaScript, it is language-independent and widely used in web APIs and configuration files."},
		{Q: "How do I validate JSON?", A: "Paste your JSON into the editor above and click the Validate JSON button. The tool instantly detects syntax errors and pinpoints the exact line and column number where the error occurs, with a helpful description of what went wrong."},
		{Q: "What does JSON formatting do?", A: "Formatting (pretty-printing) JSON adds proper indentation and line breaks to make nested structures easy to read and debug. Click Format to beautify instantly — choose 2-space, 4-space, or tab indentation."},
		{Q: "What is JSON minification?", A: "Minification removes all unnecessary whitespace and newlines to produce a compact, single-line JSON string. This reduces payload size for APIs and configuration files in production environments."},
		{Q: "What are common JSON syntax errors?", A: "The most common errors include: unquoted keys (use double quotes), trailing commas, single quotes instead of double quotes, mismatched brackets, and leading zeros in numbers. Our tool pinpoints each error with a clear description."},
		{Q: "Is my data sent to a server?", A: "No. All validation, formatting, and minification operations run entirely in your browser. Your JSON data is never transmitted to any server, ensuring complete data privacy."},
		{Q: "How do I import JSON from a URL?", A: "Switch to the Fetch URL tab, enter the URL of a JSON resource, and click Fetch. The tool will retrieve the content via a server proxy and load it into the editor, automatically formatting it."},
	}
}

// JSONSubToolFAQs returns FAQs for JSON sub-tools (regex, markdown, timestamp, base, case) by language and tool name.
func JSONSubToolFAQs(lang, tool string) []FAQ {
